...
def fill_kernel(kernel_mid_x, kernel_mid_y, kernel_size, image):
    ...
    from_x = kernel_mid_x - int(math.floor(kernel_size[0]/2))
    to_x = kernel_mid_x + int(math.floor(kernel_size[0]/2))
    from_y = kernel_mid_y -  int(math.floor(kernel_size[1]/2))
    to_y = kernel_mid_y +  int(math.floor(kernel_size[1]/2))
...
def medianFiltering(filter_size, image):
    ...
    for i in range(edge, num_rows - edge):
        for j in range(edge, num_cols - edge):
            ...
            filtered_image[i,j] = get_median(kernel.tolist())
...
def meanFiltering(filter_size, image):
    ...
    for i in range(edge, num_rows - edge):
        for j in range(edge, num_cols - edge):
            ...
            kernel = fill_kernel(i,j, filter_size, image)
            filtered_image[i,j] = get_mean(kernel.tolist())

    return filtered_image


def sample_gaussian(mean, x, variance = 0.5):
...
def fillColorKernel(kernel_mid_x, kernel_mid_y, kernel_size, image):
    ...
    kernel_b = np.zeros((kernel_size, kernel_size), dtype = int)
    kernel_g = np.zeros((kernel_size, kernel_size), dtype = int)
    kernel_r = np.zeros((kernel_size, kernel_size), dtype = int)
    kernel = list()
...
def meancolorFiltering(filter_size, image):
    ...
    for i in range(edge, num_rows - edge):
        for j in range(1, num_cols - edge):
            ...
            filtered_image[i,j,0] = get_mean(color_kernel[0].tolist())
            filtered_image[i,j,1] = get_mean(color_kernel[1].tolist())
            filtered_image[i,j,2] = get_mean(color_kernel[2].tolist())
